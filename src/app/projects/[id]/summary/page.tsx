...
import { ArrowLeft, Building2, Download, FileText, Layers, Leaf, MapPin, Package, Trash2, TrendingDown, TrendingUp } from "lucide-react";
        ...
        <div className="lg:col-span-8 space-y-12">
          <div className="space-y-6">
            <div className="flex items-center justify-between px-2">
              ...
              <Badge variant="outline" className="rounded-lg font-bold text-[10px] px-3 py-1 uppercase tracking-wider border-border/50">
                {summary.items.length} Specifications
              </Badge>
            </div>

            <div className="space-y-4">
              {summary.items.map((it) => it && (
                <Card key={it.projectMaterialId} className="group border-border/40 shadow-sm rounded-[2rem] overflow-hidden bg-background hover:border-primary/20 transition-all">
                    ...
                    <div className="flex flex-col md:flex-row justify-between gap-8">
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                          <div>
                            <div className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-widest mb-1">Quantity</div>
                            <div className="text-sm font-bold">{it.quantity} <span className="text-[10px] opacity-40">{it.unit}</span></div>
                          </div>
                          <div>
                            <div className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-widest mb-1">Lead Time</div>
                            <div className="text-sm font-bold">{it.leadTimeEstimate || "—"}</div>
                          </div>
                          <div>
                            <div className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-widest mb-1">Carbon</div>
                            <div className="text-sm font-bold text-emerald-600">{Math.round(it.carbon)} <span className="text-[10px] opacity-40 uppercase">KG</span></div>
                          </div>
                          <div>
                            <div className="text-[9px] font-black text-muted-foreground/30 uppercase tracking-widest mb-1">Budget</div>
                            <div className="text-sm font-bold text-primary">${Math.round(it.costMin)} - ${Math.round(it.costMax)}</div>
                          </div>
                        </div>

                        <div className="pt-6 border-t border-dashed border-border/40">
                          <div className="flex items-center justify-between mb-4">
                            <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Baseline Comparison</div>
                          ...
                          <div className="flex flex-wrap gap-8">
                            <div className="flex items-center gap-3">
                              <div className={cn(
                                ...
                                it.baseline.carbonDiffPct <= 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"
                              )}>
                                {it.baseline.carbonDiffPct <= 0 ? <TrendingDown className="h-4 w-4" /> : <TrendingUp className="h-4 w-4" />}
                              </div>
                              <div>
                                <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Carbon Δ</div>
                                <div className="text-xs font-bold">{(it.baseline.carbonDiffPct * 100).toFixed(1)}%</div>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              <div className={cn(
                                "h-8 w-8 rounded-xl flex items-center justify-center",
                                it.baseline.costMidDiffPct <= 0 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"
                              )}>
                                {it.baseline.costMidDiffPct <= 0 ? <TrendingDown className="h-4 w-4" /> : <TrendingUp className="h-4 w-4" />}
                              </div>
                              <div>
                                <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Cost Δ</div>
                                <div className="text-xs font-bold">{(it.baseline.costMidDiffPct * 100).toFixed(1)}%</div>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-row md:flex-col justify-end gap-2 border-t md:border-t-0 md:border-l border-border/40 pt-6 md:pt-0 md:pl-6">
                        <form action={removeMaterial}>
                          <input type="hidden" name="projectMaterialId" value={it.projectMaterialId} />
                          <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl text-muted-foreground/20 hover:text-rose-600 hover:bg-rose-50 transition-all">
        ...
        <div className="lg:col-span-4 space-y-8">
          <Card className="border-none bg-primary text-primary-foreground rounded-[2.5rem] shadow-2xl shadow-primary/20 overflow-hidden relative">
            ...
            <CardHeader className="p-10 pb-0 relative z-10">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.25em] opacity-60">Project Totals</CardTitle>
            </CardHeader>
            <CardContent className="p-10 space-y-8 relative z-10">
              <div className="space-y-1">
                <div className="text-[10px] font-black uppercase tracking-widest opacity-40">Cumulative Carbon</div>
                <div className="text-5xl font-black tracking-tighter">
                  {Math.round(summary.totals.carbon)}
                  <span className="text-xs font-bold ml-3 opacity-40 uppercase tracking-widest">kgCO₂e</span>
                </div>
              </div>
              <div className="h-px w-full bg-white/10" />
              <div className="space-y-1">
                <div className="text-[10px] font-black uppercase tracking-widest opacity-40">Budget Estimate</div>
          ...
          <Card className="border-border/40 bg-muted/5 rounded-[2.5rem] p-10 space-y-6">
            ...
            <h3 className="text-xl font-black tracking-tight">Technical Data</h3>
            <p className="text-sm font-medium text-muted-foreground leading-relaxed">
              This summary provides a consolidated view of embodied carbon and cost implications for all specified materials.
            </p>
            <div className="pt-4">
              <Button variant="outline" className="w-full h-12 rounded-xl font-bold uppercase tracking-widest text-[10px] border-border/50 bg-background hover:bg-muted">
                Detailed Export
              </Button>
            </div>
          </Card>
