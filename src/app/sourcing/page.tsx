...
export default async function SourcingPage({
  ...
  const filteredRequests = allRequests.filter(req => {
    const matchCategory = !params.category || params.category === "All Categories" || req.category === params.category;
    const matchLocation = !params.location || (req.location?.toLowerCase() || "").includes(params.location.toLowerCase());
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div>
          ...
          <p className="text-muted-foreground font-medium max-w-2xl leading-relaxed">
            Browse active material requests, track your bids, and explore market activity
        ...
        {!isSupplier && (
          (!user || isPro) ? (
            <div className="flex gap-4">
              <Link href={session ? "/sourcing/create" : "/sign-in?callbackUrl=/sourcing/create"}>
      ...
      <Tabs defaultValue="explore" className="w-full">
          ...
          <TabsList className="bg-transparent h-auto p-0 gap-8">
            ...
            {isBuyer && (
              <UpgradePrompt
                feature="Open sourcing requests and bidding"
                trigger={
                  <button type="button" className="px-0 pb-4 text-[11px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 hover:text-primary transition-all">
                    My Requests
                  </button>
                }
              />
        ...
        <TabsContent value="explore" className="mt-0">
          <div className="flex gap-2 overflow-x-auto pb-6 mb-8 scrollbar-hide">
            {["All Categories", "Concrete", "Steel", "Timber", "Glass", "Electrical"].map((cat) => (
              <Link
                key={cat}
                href={`/sourcing?${new URLSearchParams({
                  ...params,
                  category: cat,
                }).toString()}`}
              >
                <Button
                  variant={
                    (params.category === cat || (!params.category && cat === "All Categories"))
                      ? "default"
                      : "outline"
                  }
                  className={cn(
                  ...
                  <div className="flex items-center gap-4 px-2">
                    ...
                    <Badge
                      ...
                      className={cn(
                        ...
                        req.status === "open" ? "border-green-500/20 bg-green-500/5 text-green-600" : "border-amber-500/20 bg-amber-500/5 text-amber-600"
                      )}
                    >
                      {req.status}
                    </Badge>
                      ...
                      <div className="lg:col-span-4 p-10 bg-muted/30 border-r border-border/50">
                        ...
                        <div className="space-y-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider">Location</p>
                              <p className="text-sm font-bold">{req.location || "N/A"}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider">Deadline</p>
                              <p className="text-sm font-bold">{req.deadline?.toLocaleDateString() || "Open"}</p>
                            </div>
                          </div>
                        </div>

                        <div className="mt-10 pt-10 border-t border-border/50">
                          <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider mb-4">Description</p>
                          <p className="text-sm font-medium text-muted-foreground leading-relaxed line-clamp-4">
                            {req.description || "No detailed specifications provided."}
                          </p>
                        </div>

                        <Link href={`/sourcing/${req.id}`} className="block mt-10">
                          <Button variant="outline" className="w-full h-12 rounded-xl font-black uppercase tracking-widest text-[10px] gap-2 border-border/50">
                            View Full Details
                      ...
                      <div className="lg:col-span-8 p-10">
                        ...
                        {req.bids.length === 0 ? (
                          <div className="h-full flex flex-col items-center justify-center py-12 text-center bg-muted/10 rounded-[2rem] border border-dashed border-border/50">
                            <p className="text-sm font-medium text-muted-foreground">No bids submitted yet.</p>
                          </div>
                        ) : (
                              ...
                              >
                                <div className="flex items-center gap-4">
                                  <div className={cn(
                                    ...
                                    bid.status === "accepted" ? "bg-emerald-500/10 border-emerald-500/20" : "bg-muted border-border/50"
                                  )}>
                                    {bid.status === "accepted" ? (
                                  ...
                                  <div>
                                    <div className="flex items-center gap-3">
                                      <p className="text-sm font-black text-foreground">{bid.supplierName || "Anonymous Supplier"}</p>
                                      {bid.status === "accepted" && (
                                        <Badge className="bg-emerald-500 hover:bg-emerald-600 text-[8px] font-black uppercase tracking-wider px-2 py-0">Awarded</Badge>
                                      )}
                                    </div>
                                ...
                                <div className="flex items-center gap-12">
                                  <div className="text-right">
                                    <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider mb-1">Price Offer</p>
                                    <p className="text-lg font-black text-primary">${bid.bidUnitPrice}<span className="text-[10px] text-muted-foreground ml-1">/unit</span></p>
                                  </div>
                                  <div className="text-right min-w-[100px]">
                                    <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider mb-1">Lead Time</p>
                                    <p className="text-sm font-black text-foreground">{bid.leadTimeEstimate || "N/A"}</p>
                                  </div>
                                </div>
                              </div>
                            ))}
        ...
        <TabsContent value="my-requests" className="mt-0">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {myRequests.length === 0 ? (
              <div className="col-span-full">
                <Card className="border-dashed py-24 bg-muted/30 rounded-[2.5rem]">
                  <CardContent className="flex flex-col items-center justify-center text-center">
                    ...
                    <h2 className="text-xl font-bold tracking-tight">No requests created</h2>
                    <p className="text-muted-foreground mt-2 text-sm max-w-xs mx-auto">
                      You haven&apos;t created any sourcing requests yet.
                    </p>
                    <Link href="/sourcing/create" className="mt-8">
                      <Button className="rounded-xl font-black uppercase tracking-widest text-[10px]">Create Your First Request</Button>
                    </Link>
                  </CardContent>
                </Card>
              </div>
              ...
              myRequests.map((req) => (
                <Card key={req.id} className="rounded-[2rem] border-border/50 bg-background overflow-hidden group hover:border-primary/30 transition-all duration-300">
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <Badge variant="secondary" className="bg-primary/5 text-primary border-none text-[9px] font-black uppercase tracking-widest px-3 py-1">
                        {req.category}
                      </Badge>
                      <Badge variant="outline" className={cn(
                        "text-[9px] font-black uppercase tracking-wider px-3 py-1 rounded-full",
                        req.status === "open" ? "border-green-500/20 text-green-600" : "border-amber-500/20 text-amber-600"
                      )}>
                        {req.status}
                      </Badge>
                    </div>
                    <h3 className="text-xl font-black tracking-tight mb-2">{req.quantity} {req.unit || "Units"}</h3>
                    ...
                    <div className="pt-6 border-t border-dashed border-border/50 flex items-center justify-between">
                      <div className="text-[9px] font-black text-muted-foreground uppercase tracking-wider">
                        Location: <span className="text-foreground">{req.location || "N/A"}</span>
                      </div>
                      <Link href={`/sourcing/${req.id}`}>
                        <Button variant="ghost" className="h-8 px-3 rounded-lg text-[9px] font-black uppercase tracking-widest gap-2 hover:bg-primary/5 text-primary">
                          Manage
                          <ArrowRight className="h-3 w-3" />
                        </Button>
                      </Link>
            ...
            {myBids.length === 0 ? (
              <Card className="border-dashed py-24 bg-muted/30 rounded-[2.5rem]">
                <CardContent className="flex flex-col items-center justify-center text-center">
                  ...
                  <h2 className="text-xl font-bold tracking-tight">No bids submitted</h2>
                  <p className="text-muted-foreground mt-2 text-sm max-w-xs mx-auto">
                    You haven&apos;t submitted any bids for sourcing requests yet.
                  </p>
                  <Link href="/sourcing" className="mt-8">
                    <Button className="rounded-xl font-black uppercase tracking-widest text-[10px]">Browse Requests</Button>
                  </Link>
                </CardContent>
              </Card>
            ) : (
              myBids.map((bid) => (
                <Card key={bid.id} className="rounded-[2rem] border-border/50 bg-background overflow-hidden hover:border-primary/30 transition-all duration-300">
                  <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex items-center gap-6">
                      <div className={cn(
                        "h-16 w-16 rounded-[1.25rem] flex items-center justify-center border-2",
                        bid.status === "accepted" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-600" : "bg-muted border-border/50 text-muted-foreground/40"
                      )}>
                        {bid.status === "accepted" ? <Trophy className="h-8 w-8" /> : <Building2 className="h-8 w-8" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-lg font-black tracking-tight">{bid.category}</h3>
                          <Badge variant="outline" className={cn(
                            "text-[8px] font-black uppercase tracking-widest px-2 py-0",
                            bid.status === "accepted" ? "bg-emerald-500 text-white border-none" : ""
                          )}>
                            {bid.status}
                          </Badge>
                        </div>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
                          {bid.quantity} {bid.unit || "Units"} • Submitted {bid.createdAt?.toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-12">
                      <div className="text-right">
                        <p className="text-[9px] font-black text-muted-foreground uppercase tracking-wider mb-1">Your Price</p>
                        <p className="text-2xl font-black text-primary">${bid.bidUnitPrice}<span className="text-[10px] text-muted-foreground ml-1">/unit</span></p>
                      </div>
                      <Link href={`/sourcing/${bid.requestId}`}>
                        <Button variant="outline" className="h-12 rounded-xl px-6 font-black uppercase tracking-widest text-[10px] gap-2">
                          View Request
                          <ArrowRight className="h-3 w-3" />
                        </Button>
                      </Link>
